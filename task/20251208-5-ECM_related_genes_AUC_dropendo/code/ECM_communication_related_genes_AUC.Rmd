...
##drop endo
```{r}
sobj <- subset(sobj, subset = oupSample.cellType != "endo")
```
...
```{r}
subclass_list <- unique(sobj$oupSample.cellType)
subclass_list

gene_list <- ECM_communication_genesV2[[1]]
```
...
for (current_subclass in subclass_list) {
  ...
  sobj_sub <- subset(sobj, subset = oupSample.cellType == current_subclass)
  
  # 检查Group是否包含至少两个组（确保有AD和Control）
  if (length(unique(sobj_sub$oupSample.batchCond)) < 2) {
...
write.csv(auc_combined, "../Documents/Communication_gene_list_AUC_per_subclass.csv", row.names = FALSE)
```
...
# 热图参数设置（优化可读性）
pdf("../plot/gene_list_AUC_heatmap_per_subclass.pdf", width = 14, height = 8)
pheatmap(
  auc_matrix,
  main = "AUC of ECM Communication Associated Genes Across Cell Types",
