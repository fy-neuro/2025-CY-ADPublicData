# AUC Analysis for ECM_communication_genesV4 (GSE188545 Dataset)
# Script adapted from ecm_communicationV3_auc_analysis.R for V4 gene list
...
output_dir <- "task/20260108-16-ECM_communicationV4_AUC_GSE188545"
...
# ---------------------------------------------------------------------------
cat("Loading ECM communication V4 gene list...\n")
...
cat("Loaded", length(gene_list), "unique genes from ECM_communication_genesV4\n")
...
# 4. RUN AUC ANALYSIS
# ---------------------------------------------------------------------------
cat("\n=== Analyzing ECM_communication_genesV4 ===\n")
...
output_file <- file.path(output_dir, "Documents",
                         "ECM_communication_genesV4_AUC_per_celltype.csv")
...
if (nrow(auc_matrix) > 1 && ncol(auc_matrix) > 1) {
  ...
  heatmap_file <- file.path(output_dir, "plot",
                            "ECM_communication_genesV4_AUC_heatmap.pdf")
  ...
  pheatmap(
    auc_matrix,
    main = "AUC of ECM Communication Genes V4 Across Cell Types",
    color = viridis(100),
    scale = "none",
    cluster_rows = TRUE,
    cluster_cols = TRUE,
...
stats <- auc_results %>%
  filter(group == "AD") %>%
  summarise(
    Gene_List = "ECM_communication_genesV4",
...
if (nrow(high_auc_genes) > 0) {
  ...
  high_auc_file <- file.path(output_dir, "Documents",
                            "ECM_communication_genesV4_high_auc_genes.csv")
...
sink(session_file)
cat("ECM Communication V4 AUC Analysis Session Information\n")
cat("==============================================\n\n")
...
cat("## Files Generated\n\n")
cat("1. `Documents/ECM_communication_genesV4_AUC_per_celltype.csv` - Complete AUC results\n")
cat("2. `plot/ECM_communication_genesV4_AUC_heatmap.pdf` - Heatmap visualization\n")
cat("3. `Documents/auc_analysis_summary.csv` - Summary statistics\n")
cat("4. `Documents/session_info.txt` - R session information\n")
if (nrow(high_auc_genes) > 0) {
  cat("5. `Documents/ECM_communication_genesV4_high_auc_genes.csv` - Genes with AUC > 0.7\n")
}
cat("\n")
sink()
