# FINAL WORKING Pseudobulk Analysis
...
# WORKING pseudobulk analysis
run_pseudobulk_working <- function(seurat_obj, genes_of_interest, min_cells = 10) {
  ...
  for (ct in cell_types) {
    ...
    tryCatch({
      ...
      if (exists("design")) {
        cat("  Design matrix info:\n")
        cat("    Dimensions:", dim(design), "\n")
        cat("    Row names:", head(rownames(design)), "\n")
        cat("    Col names:", colnames(design), "\n")
      }
      if (exists("dge")) {
        cat("  DGEList info:\n")
        cat("    Count matrix dimensions:", dim(dge$counts), "\n")
        cat("    Group:", levels(dge$group), "\n")
      }
    })
  }
...
# Run working pseudobulk analysis
cat("\n=== RUNNING WORKING PSEUDOBULK ANALYSIS ===\n")
pseudobulk_results <- run_pseudobulk_working(sobj, gene_list)

if (nrow(pseudobulk_results) > 0) {
  # Save results
  output_file <- file.path(task_dir, "Documents", "pseudobulk_results_working.csv")
  write.csv(pseudobulk_results, output_file, row.names = FALSE)
  cat("\n✅ Saved working pseudobulk results:", output_file, "\n")
  
  # Basic summary
  cat("\n=== WORKING PSEUDOBULK SUMMARY ===\n")
  ...
  # Save summary
  summary_file <- file.path(task_dir, "Documents", "pseudobulk_summary_working.csv")
  ...
  if (any(pseudobulk_results$padj < 0.05)) {
    ...
    # Save top genes
    top_file <- file.path(task_dir, "Documents", "pseudobulk_top_significant_working.csv")
  ...
  if (file.exists(wilcoxauc_file)) {
    ...
    if (nrow(comparison) > 10) {
      ...
      # Save comparison
      comparison_file <- file.path(task_dir, "Documents", "method_comparison_working.csv")
  ...
  cat("\n❌ No working pseudobulk results generated\n")
}

cat("\n=== 🎉 FINAL PSEUDOBULK ANALYSIS COMPLETE ===\n")
