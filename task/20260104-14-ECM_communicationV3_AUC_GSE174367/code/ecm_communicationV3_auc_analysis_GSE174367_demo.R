...
# Demonstration script showing the approach for human multiome dataset
...
# ---------------------------------------------------------------------------
# 1. DEMONSTRATION - LOAD GSE174367 DATA
# ---------------------------------------------------------------------------
cat("Loading GSE174367 data for demonstration...\n")

# Try to load existing Seurat object if available
sobj_path <- "data/GSE174367/sobj_20251212.rds"
if (file.exists(sobj_path)) {
  sobj <- readRDS(sobj_path)
  cat("Loaded existing Seurat object:", ncol(sobj), "cells,", nrow(sobj), "genes\n")
} else {
  # Create a mock object for testing if no processed object exists
  cat("No processed Seurat object found. Creating a mock object for demonstration...\n")
  
  # Create a small demo Seurat object
  n_cells <- 5000
  n_genes <- 3000
  
  # Create random expression matrix
  counts <- Matrix::Matrix(rpois(n_cells * n_genes, lambda = 5), 
  ...
  sobj <- CreateSeuratObject(
    counts = counts,
    project = "GSE174367_demo"
  )
  
  # Add mock metadata
  sobj$Diagnosis <- sample(c("AD", "Control"), n_cells, replace = TRUE, prob = c(0.6, 0.4))
  sobj$Cell.Type <- sample(c("Excitatory", "Inhibitory", "Astrocyte", "Microglia", "Oligodendrocyte"), 
                           n_cells, replace = TRUE)
  
  cat("Created demo Seurat object:", ncol(sobj), "cells,", nrow(sobj), "genes\n")
}

# Check if Diagnosis metadata exists
if (!"Diagnosis" %in% colnames(sobj@meta.data)) {
  cat("Diagnosis column not found in metadata. Adding mock diagnosis for testing.\n")
...
# ---------------------------------------------------------------------------
cat("\n=== Demonstrating ECM_communication_genesV3 AUC analysis (GSE174367) ===\n")
...
output_file <- file.path(output_dir, "Documents", 
                         "ECM_communication_genesV3_AUC_per_celltype_GSE174367_demo.csv")
...
if (nrow(auc_matrix) > 1 && ncol(auc_matrix) > 1) {
  ...
  heatmap_file <- file.path(output_dir, "plot", 
                            "ECM_communication_genesV3_AUC_heatmap_GSE174367_demo.pdf")
  ...
  pheatmap(
    auc_matrix,
    main = "AUC of ECM Communication Genes V3 Across Cell Types (GSE174367 - Demo)",
    color = viridis(100),
    scale = "none",
    cluster_rows = TRUE,
    cluster_cols = TRUE,
...
summary_file <- file.path(output_dir, "Documents", "auc_analysis_summary_GSE174367_demo.csv")
...
# ---------------------------------------------------------------------------
summary_file <- file.path(output_dir, "Documents", "analysis_summary_GSE174367_demo.md")
sink(summary_file)
...
cat("## Overview\n\n")
cat("This task demonstrates the approach for AUC analysis to identify ECM communication V3 genes that show differential expression between conditions across different brain cell types in GSE174367 human multiome dataset. The actual analysis uses area under the receiver operating characteristic (ROC) curve to quantify each gene's ability to distinguish between conditions within each cell type.\n\n")
cat("## Demonstration Notes\n\n")
cat("This is a demonstration using simulated data. For actual analysis:\n\n")
cat("1. **Cell Type Annotation**: Ensure proper cell type annotation (Cell.Type) is available in dataset\n")
cat("2. **Condition Metadata**: Ensure condition metadata (Diagnosis: AD vs Control) is available\n")
cat("3. **Data Loading**: Properly load and process the multiome data\n")
...
cat("- AUC calculated for AD vs Control comparison\n\n")
...
cat("## Files Generated\n\n")
cat("1. `Documents/ECM_communication_genesV3_AUC_per_celltype_GSE174367_demo.csv` - Demonstration AUC results\n")
cat("2. `plot/ECM_communication_genesV3_AUC_heatmap_GSE174367_demo.pdf` - Demonstration heatmap\n")
cat("3. `Documents/auc_analysis_summary_GSE174367_demo.csv` - Demonstration summary statistics\n")
cat("4. `Documents/analysis_summary_GSE174367_demo.md` - This summary document\n\n")
cat("## Next Steps for Real Analysis\n\n")
cat("To perform actual AUC analysis:\n\n")
cat("1. **Load Data**: Properly load GSE174367 multiome data (snRNA-seq + snATAC-seq)\n")
cat("2. **Process Data**: Apply normalization, scaling, and dimensionality reduction\n")
cat("3. **Cell Type Annotation**: Ensure Cell.Type metadata is properly annotated\n")
cat("4. **Quality Control**: Filter cells and genes as appropriate\n")
cat("5. **Batch Correction**: Apply Harmony or similar method if needed\n")
cat("6. **Run Analysis**: Execute the full AUC analysis pipeline\n")
cat("7. **Interpret Results**: Identify genes with high AUC values for follow-up\n\n")
cat("## References\n\n")
cat("This analysis follows the same methodology used in:\n")
cat("- `20251214-11-GSE188545_AUC` (successful analysis on GSE188545)\n")
cat("- `20251212-7-GSE174367_AUC` (previous ECM gene analysis on GSE174367)\n\n")
sink()

cat("\n=== Demonstration complete ===\n")
cat("Outputs saved to:", output_dir, "\n")
cat("Note: This is a demonstration using simulated data.\n")
cat("For actual analysis, ensure proper data loading and preprocessing.\n")